...
function parseInput(val) {
  ...
  return Math.max(10, Math.min(5999, mins * 60 + secs))
}

export default function BreathingTimer({ pattern, onClose }) {
  const defaultDur = pattern.defaultDuration ?? 300
  ...
  // Session duration
  const [sessionSecs, setSessionSecs] = useState(defaultDur)
  const [countdown, setCountdown]     = useState(defaultDur)
  const [durInput, setDurInput]       = useState(secsToDisplay(defaultDur))
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setElapsed(prev => {
        const next = prev + 0.1
        if (next >= phaseDuration) { nextPhase(); return 0 }
        return next
      })
    }, 100)
  ...
  // Session countdown interval (1 s ticks)
  useEffect(() => {
    if (!running) return
  ...
  const toggle = () => {
    if (!running) {
      if (countdown === 0) {
        // Restart: reset countdown to current sessionSecs
        setCountdown(sessionSecs)
        setPhaseIndex(0)
        setElapsed(0)
        setCycleCount(0)
      }
  ...
  const handleDurBlur = () => {
    const secs = parseInput(durInput)
    setSessionSecs(secs)
    setCountdown(secs)
    setDurInput(secsToDisplay(secs))
  }

  const handleDurKeyDown = (e) => {
  ...
  const countdownPct = sessionSecs > 0 ? countdown / sessionSecs : 0

  return (
    <div className="flex flex-col items-center gap-5 py-4">

      {/* Session duration input — only when stopped */}
      {!running && !finished && (
        <div className="flex items-center gap-3 w-full">
          <span className="text-xs text-slate-500 uppercase tracking-widest flex-shrink-0">Übungsdauer</span>
          <input
            type="text"
            inputMode="numeric"
            value={durInput}
            ...
            className="w-24 text-center text-lg font-mono font-semibold bg-slate-800/60 border border-slate-600/50 rounded-lg px-3 py-1.5 text-white focus:outline-none focus:border-blue-500/70"
            placeholder="05:00"
          />
          <span className="text-xs text-slate-600">min:sek</span>
        </div>
      )}

      {/* Completion message */}
      {finished && (
        <div className="w-full rounded-xl border border-teal-500/40 bg-teal-500/10 px-4 py-3 text-center text-teal-300 text-sm font-medium">
          Übung beendet – Gut gemacht! 🌊
        </div>
      )}

      {/* Countdown display — shown while running or when countdown < sessionSecs */}
      {(running || countdown < sessionSecs) && !finished && (
        <div className="flex items-center gap-3 w-full">
          {/* Progress bar */}
          <div className="flex-1 h-1.5 bg-slate-700/50 rounded-full overflow-hidden">
            <div
              ...
              style={{
                ...
                background: countdownPct > 0.33 ? '#3b82f6' : countdownPct > 0.1 ? '#f59e0b' : '#ef4444',
              }}
            />
          </div>
          <span className="text-2xl font-mono font-light text-white tabular-nums flex-shrink-0">
            {secsToDisplay(countdown)}
          </span>
        </div>
      )}
      ...
      <div className="text-center">
        <p className={`text-lg font-semibold ${colors.label}`}>{currentPhase.name}</p>
      ...
      <div className="w-full grid grid-cols-4 gap-2 mt-1">
        {phases.map((p, i) => (
          <div key={i} className={`rounded-lg p-2 text-center text-xs border transition-all ${
            i === phaseIndex
